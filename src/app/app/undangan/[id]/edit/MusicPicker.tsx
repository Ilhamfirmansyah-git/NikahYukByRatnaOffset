...
export default function MusicPicker({ value, onChange }: MusicPickerProps) {
  ...
  const matchingPreset = songs.find(s => s.url === value.url);
  ...
  function togglePreview(e: React.MouseEvent, song: SongItem) {
    e.stopPropagation();
    if (playingId === song.id) { stopPreview(); return; }
    stopPreview();
    ...
    audio.play()
      .then(() => { setLoadingId(null); setPlayingId(song.id); })
      .catch(() => {
        stopPreview();
        toast.error(`Gagal memutar "${song.title}".`);
      });
  }
  ...
  function selectCustom() {
    stopPreview();
    if (!isCustom) onChange({ ...value, url: '', judul: '' });
  }
  ...
  return (
    <div className="space-y-4">
      {fetchingSongs ? (
        ...
        <div className="p-6 bg-cream-50 rounded-xl border border-cream-200 text-center">
          <p className="text-sm text-gray-500 mb-1">Belum ada lagu tersedia.</p>
          <p className="text-xs text-gray-400">Admin perlu upload lagu terlebih dahulu di menu <strong>Admin → Lagu</strong>.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {songs.map(song => {
            ...
            const catColor = CATEGORY_COLOR[song.category] ?? 'bg-gray-100 text-gray-600';
            return (
              <button
                key={song.id}
                type="button"
                ...
                className={`relative text-left p-3 rounded-xl border-2 transition-all ${
                  isSelected ? 'border-primary bg-primary-50' : 'border-cream-200 bg-white hover:border-primary/40 hover:bg-cream-50'
                }`}
              >
                {isSelected && (
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${catColor}`}>{song.category}</span>
                  <button
                    type="button"
                  ...
                  >
                    {isLoading ? (
                      ...
                      <svg className="w-2 h-2" fill="currentColor" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></svg>
                    ) : (
          ...
          <button
            ...
            className={`relative text-left p-3 rounded-xl border-2 transition-all ${
              isCustom ? 'border-primary bg-primary-50' : 'border-dashed border-cream-300 bg-white hover:border-primary/40 hover:bg-cream-50'
            }`}
          >
            {isCustom && (
              <div className="absolute top-2 right-2 w-4 h-4 bg-primary rounded-full flex items-center justify-center">
      ...
      <label className="flex items-center gap-3 cursor-pointer">
        <div className="relative">
          <input type="checkbox" className="sr-only" checked={value.autoplay} onChange={e => onChange({ ...value, autoplay: e.target.checked })} />
