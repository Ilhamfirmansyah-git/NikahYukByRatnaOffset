...
import InvitationClient from '@/app/u/[slug]/InvitationClient';
import { InvitationData } from '@/types/invitation';

interface PageProps {
  params: { prefix: string };
  searchParams?: { to?: string; tamu?: string };
}

async function getInvitationByPrefix(prefix: string) {
  return prisma.invitation.findUnique({
    where: { customDomain: prefix },
    include: {
      template: { select: { componentKey: true, name: true } },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const invitation = await prisma.invitation.findUnique({
    where: { customDomain: params.prefix },
...
export default async function SubdomainPage({ params, searchParams }: PageProps) {
  const { prefix } = params;
  const guestName = searchParams?.to || searchParams?.tamu || '';

  const invitation = await getInvitationByPrefix(prefix);
